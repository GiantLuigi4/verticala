...
public class OverworldChunkDecoratorCubic implements ChunkDecoratorCubic {
	@Override
	public void decorate(Chunk chunk, GenerationGroup group) {
		for (int yPos = group.yPosition; yPos < group.yPosition + 8; yPos++) {
			if (yPos == -1) {
					...
					for (int z = 0; z < 16; z++) {
						...
						for (int y = -5; y < 0; y++) {
							if (y >= type.getMinY() - rand.nextInt(5)) {
								group.setBlock(x, (7 << 4) + y + 16, z, Block.bedrock.id);
							}
						}
					}
				}
